...
create table ItemCompra (
	...
	fkcomprat integer,
	fkhistoricoP integer,
	FOREIGN key (fkcomprat) REFERENCES comprat (id),
	FOREIGN key (fkhistoricoProd) REFERENCES historicoProd (id)
);

create table historicoProd (
id serial primary key, 
fkproduto varchar (200) not null, 
fkTem int null,
validade date null,
...
atualizado_em TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
FOREIGN Key (fkproduto) references produto (codigo), 
FOREIGN Key (fkTem) references tem (id)
);

CREATE TRIGGER trigger_atualiza_data_modificacao
BEFORE UPDATE ON historicoProd
FOR EACH ROW
EXECUTE FUNCTION atualiza_horarioDia();  